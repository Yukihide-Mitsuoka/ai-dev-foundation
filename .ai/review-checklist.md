...
## id: review-checklist title: AI Review Checklist authority: 4 read_when: [review, self-review, pr]
...
1. Read the PR description and the linked issue first — review against *intent*, not
   just the diff.
1. Classify the change scope (ARC-020) and check the matching rules were followed.
1. Walk the checklist below. Skip sections that clearly don't apply, but say so.
1. Report findings ranked by severity: **Blocker** (guardrail/security/correctness) >
   **Major** (bug risk, missing tests) > **Minor** (quality) > **Nit** (style beyond
   linter — usually omit).
1. Every finding cites: file:line, the violated rule ID if any, and a concrete fix.
   Findings without an actionable fix are observations, not findings.
1. Verify claims by reading the code — never approve on the diff description alone
