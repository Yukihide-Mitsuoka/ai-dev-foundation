...
## name: bugfix description: Diagnose and fix incorrect behavior with a regression test that pins the fix triggers: [bug, defect, incorrect behavior, crash, regression] reads: [.ai/workflow.md, .ai/testing.md]
...
1. Write the regression test that encodes the *expected* behavior. Run it — it MUST
   fail, and fail for the right reason. Commit it first (`test(scope): reproduce #N`).
1. Diagnose the root cause: trace from symptom to cause; state the cause in one
   sentence. If you can't, you're not ready to fix.
1. Fix at the cause, not the symptom. Minimal diff — resist drive-by refactoring
   (COD-021); file follow-up issues instead.
1. Run the regression test (now green) + the module's full suite + `make test`.
1. Sweep for siblings: search for the same pattern elsewhere in the codebase; fix in
   the same PR only if identical and small, otherwise open issues.
1. Update `docs/troubleshooting/` if users could hit this; runbook if ops action exists.
1. PR with `fix(scope):` title; description includes root cause + why this fix.
