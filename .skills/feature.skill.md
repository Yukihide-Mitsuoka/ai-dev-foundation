...
## name: feature description: Implement new functionality end-to-end (requirements → design → code → tests → docs → PR) triggers: [new feature, add functionality, implement issue] reads: [.ai/workflow.md, .ai/architecture.md, .ai/coding-rules.md, .ai/testing.md]
...
1. Classify impact (ARC-020). Architectural → switch to architecture.skill.md first.
1. Plan the seam: which layer does each piece belong to? New use case in
   `application/`, domain logic in `domain/`, I/O in `infrastructure/` adapters.
1. Create branch `feat/<issue>-<slug>`.
1. Implement in thin vertical slices: domain → application → interface, committing
   per slice with tests in the same commit.
1. Cover error paths and boundaries (TST-002), not just the happy path.
1. Update docs per the doc-update matrix (DOC-030): MODULE.md, `docs/api/`,
   `.env.example`, glossary.
1. Run `make format && make lint && make test`.
1. Self-review with review.skill.md; then open the PR with the template fully filled.
