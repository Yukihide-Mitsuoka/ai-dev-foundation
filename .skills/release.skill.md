...
## name: release description: Prepare and verify a release; humans approve, agents prepare triggers: [release, ship, publish version, prepare release] reads: [.ai/release.md, .ai/security.md]
...
1. Audit the changelog the automation generated: does every entry match an actual
   ...
   generated output.
1. Run the gate table (REL-020): tests, CodeQL, Trivy, gitleaks, licenses, SBOM,
   container scan. Record each result verbatim (GR-042).
1. Docs sweep: README quickstart still true? `docs/deployment/` current? New env vars
   in `.env.example`? Migration notes written for any breaking change?
1. Rollback readiness: is the release revertible (REL-040)? For risky releases, write
   the rollback steps into `docs/runbook/` before, not after.
1. Write the risk summary on the Release PR: what changed, blast radius, gates status,
   rollback plan, anything the human should weigh.
1. Hand off: request human approval. **Do not merge the Release PR yourself** (REL-010).
1. After the human merges: verify the release workflow completed — tag, artifacts,
