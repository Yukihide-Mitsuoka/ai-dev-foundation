...
## name: security description: Security hardening, vulnerability response, and security review of changes triggers: [vulnerability, CVE, security review, hardening, auth change, pentest finding] reads: [.ai/security.md, .ai/guardrails.md, SECURITY.md]
...
1. For leaked secrets: rotation FIRST (the secret is compromised forever — SEC-040),
   then history cleanup, then the process fix that prevents recurrence.
1. Fix at the trust boundary: validate input (SEC-010), enforce authZ in the use case
   (SEC-020), parameterize queries, encode output. Prefer platform/library security
   primitives over hand-rolled logic (SEC-012).
1. Add a test that encodes the security property (e.g. "unauthenticated request to X
   returns 401", "path traversal input is rejected").
1. For dependency CVEs: upgrade > patch > mitigate-and-document, in that order (SEC-030).
1. Verify with the scanner that flagged it; run `make security-scan`.
1. Record: decision log entry; `docs/troubleshooting/`/runbook if operational.
